...
> **阶段** 5 / 6　·　**前置**：阶段 1–4　·　**产出**：把整合主线补全、做论文能力的机制级对应实验、并把观察升级为可测证据
...
flowchart LR
    ...
    T3["Task 3 · 注释迁移<br/>A: zero/full-shot；B/P: zero-only"] --> OK3["A 可直对；B/P 扩展"]
...
| **E1** | 注释迁移（A: zero/full-shot；B/P: zero-shot）+ kNN 对照 | Ext. Data Fig. 2g,h | A 的 AUROC 0.891 可与论文 drop-5% 对照；B/P 只作域外压力测试 |
| **E3** | 批不变编码器"全局置换 batch"探针 | Methods（编码器 F(X)） | 覆盖45位患者且100%换batch；scAtlasVAE结构上Δz≡0 |
...
*图 5-E1 — 注释迁移结果。**只有设计 A 同时包含 zero-shot 与 full-shot**；设计 B/P 没有 full-shot 柱，均只做 zero-shot。图中 scVI kNN 为公平的 `reference-only frozen encoder` 结果；下表另外保留旧的 full-data transductive kNN，仅用于诊断和追溯，二者不能混为同一基线。*
...
> **上表采用官方源码默认日程**（`pred_last_n_epoch=10`；不同 reference 规模自动得到约 76–94 个 epoch，P 为 81），分类头只训末 10 轮。论文 Methods 没有明确披露这个参数，不能把它写成“论文协议”。A/B 的旧 fulltime 对照 AUROC 为 0.941/0.907；P 的 150/150 对照为 **0.575/0.305/0.880**（源码默认日程为 **0.489/0.215/0.851**）。注意 P 对照同时改变总训练轮数（81→150）和分类头启用轮数（10→150），只能视为联合日程对照，不能把提升单独归因于分类头全程启用。`fulltime` 仍**不是** query/reference 共训的 `full-shot`。

![设计 P 的训练日程敏感性](figures/fig_phase5_transfer_patient_protocol.png)

*图 5-E1b — 同一个整 patient 留出下，比较 zero-shot 的源码默认自动总轮数/末10轮与 fulltime 150/150轮；两者均不训练 query，但同时改变了两个日程变量，因此只作联合对照。*
...
- **⚠️ 一处方法学自我更正（本轮复查）**：`pred_last_n_epoch=10` 是官方源码默认值，并非论文 Methods 明确报告的协议。源码默认日程下 A/B/P AUROC 为 **0.891/0.851/0.851**；旧 fulltime 联合日程对照为 **0.941/0.907/0.880**。这些对照不能写成“超越论文”，也不改变 A/B/P 留出单位的证据边界。
...
- **设计 A 的单次运行中 zero-shot 接近 full-shot**（AUROC 0.891 vs 0.905），方向与论文“zero-shot 已足够好、不必共训”一致；这不代表 B/P 也做过 full-shot，也不构成跨 seed 稳健性证明。zero-shot **不重训 query**、更省算力，是该 encoder 接口的实际优势。
...
2. **分类头默认只在最后 `pred_last_n_epoch`(=10) 个 epoch 才训练**。这个“末 10 轮”来自源码默认，论文 Methods 未明确披露；它会随自动总 epoch 改变相对占比。当前 A/B/P 源码默认日程 AUROC 为 0.891/0.851/0.851；旧 fulltime 仅作联合日程对照。`fulltime` 与只对设计 A 做过的 `full-shot` 是两个不同概念。
...
阶段 1/3 我们一直说 scAtlasVAE 的题眼是"编码器只吃 X、不看 batch"，证据是 `_gex_model.py` 的 `encode()` 中“把 batch 拼进编码器输入”的语句**被注释掉了**（当前工作副本审计时约在 `991–994` 行；以语句搜索为准）。这一阶段把它做成一个**可测的实验**。

**做法**（脚本 `phase5_batch_invariance_probe.py`）：从全对象近似均衡分层抽取 8,000 个细胞，覆盖 45/45 位患者；随后在患者名称层面构造保持 batch 边际计数、但令 8,000/8,000 个细胞都换 batch 的全局多重集错排，再分别翻译成各模型的内部 code。分别用**真实 batch / 全局置换 batch / 全 None**过编码器，比较潜均值。scAtlasVAE 端显式关闭内部 dataset shuffle，确保保存的 AnnData obs 索引就是实际探针行。NPZ 保存全部探针 latent、obs 索引和统一的规范 batch code；验证器会回查 H5AD patient、从 NPZ 重算漂移并与 CSV 对齐。

**结果（本机实测）**：

![批不变探针](figures/fig_phase5_invariance.png)

| 编码器 | 全局置换 batch 后 max\|Δz\| | 平均 L2 漂移 |
|---|---|---|
| **scAtlasVAE**（结构上不吃 batch） | **0.0** | **0.0** |
| scVI（默认 `encode_covariates=False`） | 0.0 | 0.0 |
| scVI（`encode_covariates=True`，吃 batch） | **0.460** | **0.217** |

**门道（一个漂亮的、有洞察的三方对照）**：
- **scAtlasVAE 全局置换 batch 后 q_mu 逐元素完全不变（Δ 精确为 0）**——坐实"编码器结构上无视 batch"。旧版批内置乱因数据按 patient 排序，仅 2.625% 细胞真正换 batch，且 scAtlasVAE 内部 dataset shuffle 曾让保存的 obs 索引与实际输入行错位；两项均已废弃。修复后的 100% 全局错排让 scVI 编码 batch 对照的平均漂移由被稀释的 0.0069 恢复为 0.217，并由 H5AD 反查索引—患者关系。
...
## Task 2 · 跨图谱整合 + 多头标签对齐（机制级对应实验，Ext. Data Fig. 3）
...
*图 5-E6 — 同一次前向中，Yost 与 Zheng 两个分类头对所有细胞的预测共现矩阵（行归一化）。为避免低值文字拥挤，图中只给 **>0.15** 的格子显示数值；未标数字不等于 0，完整矩阵见 [`phase5_cross_atlas_head_alignment.csv`](../data/phase5_cross_atlas_head_alignment.csv)。它是论文式多头标签对齐；latent-kNN 另存为附加诊断，不再冒充该结果。*
...
- **E1 训练协议**：主结果使用官方源码默认的分类头末10轮；论文Methods未明确报告该参数。P 的源码默认81/10与fulltime 150/150同时改变总轮数和分类头轮数，不能隔离单一日程效应；两者都不是full-shot，也不把patient留出变成study留出。
...
- **随机性**：本阶段主结果与扩展均为单次运行，没有多 seed 均值/误差条；各脚本也未在所有模型构造前统一锁定 Python/NumPy/Torch/CUDA 随机状态。版本与 GPU 浮点顺序还会带来额外波动，因此小分差只作描述性结果，不能声称跨 seed 稳健。
