...
![从泊松到 ZINB 的三步搭建](fig_zinb_construction.svg)
...
![编码器 batch 位置对比](fig_batch_invariant_vs_scvi.svg)

*图 1-2 — 四种 VAE 方法的关键差别只在一列：编码器输入看不看 batch。只有 scAtlasVAE（和 SCALEX）不看。*
