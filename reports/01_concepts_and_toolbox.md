...
> **深入（可选）· 一个"读代码才知道"的反常识**：论文说预热贯穿训练，但真去读 `fit()` 源码（`_gex_model.py:1243`）会发现默认 `n_epochs_kl_warmup=400`，而 11 万细胞时训练总共才约 73 个 epoch——`kl_weight = min(1, epoch/400)` 意味着**训练到结束，KL 权重最高才 ≈0.18，从没升到过 1**。这既解释了模型为何敢用 ZINB 这种强重构（KL 一直很弱、不会压垮它），也让"关掉预热 vs 缩短预热"成为[阶段 4](phase4_ablation_studies.md) 更贴代码事实的消融。这类"论文没写、代码才有"的发现，正是复现的价值所在。
