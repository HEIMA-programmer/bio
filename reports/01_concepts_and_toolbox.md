...
> **导航**：[← 总纲](00_overview_and_learning_map.md)　·　[索引](README.md)　·　[阶段 1 →](phase1_environment_setup.md)
...
```mermaid
flowchart LR
    subgraph AE["自编码器 AE：细胞 → 一个确定的点"]
        direction LR
        Xa["细胞 X"] --> Ea["编码器"] --> Pa["潜空间<br/>一个确定的点<br/>点间留洞、不连续"] --> Da["解码器"] --> Ra["重构 X̂"]
    end
    subgraph VAE["变分自编码器 VAE：细胞 → 一团云，再采样"]
        direction LR
        Xv["细胞 X"] --> Ev["编码器"] --> Mv["μ, σ²<br/>一团高斯云"] --> Sv["采样<br/>z = μ + σ·ε"] --> Dv["解码器"] --> Rv["重构 X̂"]
        Mv -. "KL 把每团云拉向 N(0,I)<br/>→ 平滑、连续、有组织" .-> Sv
    end
```

*图：左 AE 把细胞压成孤立的点（点间留洞、不连续）；右 VAE 压成一团高斯云、从中采样，再用 KL 把所有云拉向 N(0, I)——换来平滑、有组织、可迁移的潜空间。*
...
把这三步画出来，一眼就懂它们各自在治什么"病"：

```mermaid
flowchart LR
    P["① 泊松 Poisson<br/>方差 = 均值<br/>太死板，套不住"] -->|"加 gamma 混合<br/>→ 允许方差 > 均值"| NB["② 负二项 NB<br/>均值 μ + 离散度 θ<br/>治过离散"]
    NB -->|"并联补零开关 π<br/>→ 解释过多的 0"| ZINB["③ 零膨胀负二项 ZINB<br/>μ, θ, π 三参数<br/>治 dropout，解码器最终用它"]
```

*图 1-1 — ①泊松（方差=均值，太死板）→ ②负二项（加离散度 θ，方差>均值）→ ③零膨胀（再并联补零开关 π，治 dropout 造成的过多零）。*

> **去哪看代码**：这个分布不是抽象概念，仓库里真有它的实现。`grep -rn "class ZeroInflatedNegativeBinomial" scAtlasVAE/scatlasvae/utils/_distributions.py` 能定位到分布类；`utils/_loss.py:113` 的 `zinb_reconstruction_loss` 则是"拿真实计数问这个分布：你给它多大概率"。[阶段 3](phase3_reimplement_vae.md) 会带你逐行读这段负对数似然。
...
>
> **什么是归一化 (normalization)**：把数值缩放到统一尺度、方便比较。`normalize_total + log1p` 是 PCA/常规 Scanpy 分析常用路径，但 **scAtlasVAE 默认不是这条路径**：`adata.X` 传入原始计数，encoder 内部因 `log_variational=True` 只做 `log1p`；只有显式设置默认关闭的 `total_variational=True` 才会再按文库大小归一化。§3 的"试一试 2"用于理解通用预处理，不要把它误写成模型默认输入。
>
> **常见坑**：`log_variational=True` 的含义是模型在 encoder 内对原始计数做 `log1p`，不是预先执行 `normalize_total`。ZINB 重建目标仍是原始整数计数；本项目走默认路径时 `adata.X` 必须保留整数且每细胞总计数 > 0。
...
- **训练细节**：源码默认 `pred_last_n_epoch=10`，分类损失只在**最后 10 轮**进入优化目标。这是源码默认日程，不是论文 Methods 明确披露的协议；本项目把它与全程启用分类损失的设置分别标注，不能把二者差异预先解释成“必然更稳”。
...
> **深入（可选）· 一个"读代码才知道"、且要读**全**的细节**：论文说预热贯穿训练。真去读 `fit()` 会看到默认 `n_epochs_kl_warmup=400`，随后又执行 `n_epochs_kl_warmup = min(max_epoch, n_epochs_kl_warmup)`。所以当训练轮数 `max_epoch < 400`（例如 4 万细胞 →200；**本项目 10.5 万细胞 →76**），预热长度被**截断成 max_epoch**；官方实现首轮权重为 0、每轮末递增，最终升到约 1。（曾有一版报告只读到函数签名的 400、漏了这行 `min`，误以为"λ_KL 只到 ≈0.18、从没到 1"——我们在 4060 上实跑的 λ_KL 曲线证实它爬到了 ~1，见[阶段 2 图 2-2](phase2_integration_and_benchmark.md)。）含义仍在：**早期 λ_KL 很小、重构（ZINB）主导，末期才到满强度**。行号会随本地修复变化，复查时应搜索函数名与上述赋值语句，不要依赖固定偏移。

---

### 1.5 scAtlasVAE 的设计：encoder 是否显式接收 batch（及与 scVI 配置的区别）

市面上已有 scVI、scANVI、SCALEX、scPoli 等 VAE 方法。scAtlasVAE 的源码把 encoder 固定为只接收 X。论文用下面的架构表概括各方法；需要注意，它描述的是一般形式，不等同于本项目每个软件包的默认运行配置：

| 方法 | 编码器 | 解码器 | 重构 |
|---|---|---|---|
| **scAtlasVAE** | `F(X) → z`（**只吃基因表达**） | `F(z, B) → (r_mean, r_var, r_gate)` | ZINB |
| scVI / scANVI（论文概括） | `F(X, B, S) → (z, z_l)` | `F(z, z_l, B) → ...` | ZINB |
| SCALEX | `F(X) → z` | `F(z, B) → X̃` | BCE |
| scPoli | `F(X, B) → z`（吃了批次 B） | `F(z, B) → ...` | ZINB |

*软件实现细节：scvi-tools 默认 `encode_covariates=False`，所以本项目实际 scVI encoder 也不显式接收 batch；只有显式设为 `True` 才会接收 covariate。阶段 5 的打乱-batch 探针验证的是这三个实际配置，而不是照抄论文表格。*

- **不显式接收 batch 的 encoder**：scAtlasVAE encoder 输入只有 X，batch 只在解码端注入。这是结构接口保证；它不保证表达矩阵 X 中携带的批次信号一定从 z 中消失。

> **动手验证（别只信这句话）**：打开 `scAtlasVAE/scatlasvae/model/_gex_model.py`，搜索 `def encode`，再找紧邻的 `X = torch.hstack([X, batch_index])`；它**被注释掉了**。当前工作副本审计时位于约 `991–994` 行，但本地修复会改变行号，所以以函数名和语句搜索为准，不再假设“整体 +3”。作者写了它、又划掉了——这是明确的结构设计。[阶段 3](phase3_reimplement_vae.md) 会带你亲眼看这一行。
- **这带来什么能力（zero-shot 接口）**：新 query 可以直接过同一个 scAtlasVAE encoder，不必为未知 batch 扩展 encoder 输入。本项目也验证了默认 scVI reference encoder 可直接投影 query counts、完全不调用 `qm.train()`；因此正确区别是 scAtlasVAE **结构上固定**不接收 batch，而 scVI **可配置且默认也不接收**，不能写成“scVI 新 batch 必须架构手术”。
...
> **试一试 2 — 归一化前后对比**：这是 Scanpy/PCA 常用的 `normalize_total(1e4) + log1p` 练习，用于理解文库大小归一化；scAtlasVAE 默认 encoder 仅在模型内部 `log1p(raw counts)`，不要把两条路径混为一谈。
...
- 与 scVI 比较时的题眼：scAtlasVAE encoder **结构上固定只接收 X**；scVI encoder 的 covariate 输入可配置，本项目默认也不接收 batch。zero-shot 是否有效还要看真实迁移指标。
