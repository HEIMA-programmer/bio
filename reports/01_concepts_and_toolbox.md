...
- **负二项 (Negative Binomial, NB)**：加一个**离散度 (dispersion)** 旋钮，让**方差可以大于均值**（过离散，真实 scRNA 就这样）。
- **零膨胀负二项 (Zero-Inflated NB, ZINB)**：在 NB 上再加一块"额外补零"，解释单细胞里过多的零（**dropout**：基因表达了却没被测到）。

于是解码器为每个基因输出**三个参数**：① **均值比例 (scale)**——对所有基因做 softmax 得占比，再乘该细胞的**文库大小 (总计数)**，得计数尺度的均值 μ；② **离散度 (dispersion, θ)**；③ **零膨胀门控 (dropout/gate, π)**。

> **文库大小为什么要乘进去**：不同细胞测序深浅不同（有的测到 2000 条、有的 8000 条）。解码器先出各基因的相对占比（加起来＝1），再乘该细胞的总计数，才还原成它真实的计数尺度。
>
> **常见坑**：编码器的**输入**先做 `log(1+x)` 归一化（`log_variational=True`），但解码器**重建目标是原始整数计数**——两者别混。且走 ZINB 时 `adata.X` 必须是整数、每细胞总计数 > 0，否则出 NaN。
...
- **重构损失**：真实计数在解码器预测的 ZINB 分布下的"不可能程度"（术语叫**负对数似然**——似然越高、损失越低）。
- **KL 损失**：每团云离 N(0, I) 多远——就是 (e) 讲的那两只手。λ_KL 是它的权重。
- **分类损失（半监督，可选）**：有细胞类型标签时，在潜向量 z 上接一个小分类器，用**交叉熵 (cross-entropy)** 衡量分类准不准。λ_ct 默认 1。

重构与 KL 是一对拉锯：重构要潜空间**多带信息**、KL 要它**规整**，平衡好才得到既有用又规整的表示。

> 一句话记住：**损失 ＝ 重建好不好 + 潜空间规不规整 +（可选）分类准不准；训练就是让这三块的加权和变小。**

#### (h) KL 预热 (warmup)：别让潜空间过早坍缩

> **深入（可选）——后验坍缩 (posterior collapse)**：一开始就给 KL 项很大的权重，模型会图省事，让潜空间直接坍缩成 N(0, 1)、**根本不携带信息**（因为那样 KL 损失最小）。解决办法是让 λ_KL 从 0 **慢慢线性升到目标值**——先学会用潜空间重构，再逐步收紧正则。论文里这个预热贯穿整个训练。**关掉预热**是阶段 4 一个很好的消融实验。
