...
回到解码器这一半。它要重建"原始计数"，但单细胞计数很特别：**整数、极稀疏、方差大、零特别多**。所以解码器**不直接输出一个数**，而是输出一个**概率分布的参数**，我们再问"真实计数在这个分布下有多大可能"——可能性越高＝重建越好。这个分布是 **ZINB**，一步步拼出来：

- **泊松 (Poisson)**：最简单的计数分布，但强制"方差＝均值"，太死板。
- **负二项 (Negative Binomial, NB)**：给泊松加一个**离散度 (dispersion)** 旋钮，让**方差可以大于均值**。这里的"离散度"就是"数据比泊松更散多少"的度量——**离散度越大，同样的平均表达量下、各细胞的计数越参差不齐**。真实 scRNA 正是这种"过离散 (overdispersion)"，所以 NB 比泊松更贴合。
- **零膨胀负二项 (Zero-Inflated NB, ZINB)**：在 NB 上再加一块"额外补零"，专门解释单细胞里**过多的零**（**dropout**：基因其实表达了，却没被测到）。

于是解码器为每个基因输出**三个参数**，一一对应上面：

1. **均值比例 (scale)**：先对所有基因做一次 **softmax**——softmax 就是"把一组任意数字压成加起来正好等于 1 的占比"（像把各科原始分换算成百分比份额）。这样得到"这个细胞里各基因各占多少表达"的占比，再乘该细胞的**文库大小 (library size，即总计数)**，就还原成计数尺度的均值 μ。
2. **离散度 (dispersion, θ)**：上面那个"过离散"旋钮，控制方差比均值大多少。
3. **零膨胀门控 (dropout gate, π)**：一个"开关式"的概率——**额外多大几率直接吐一个 0**（不管负二项那部分算出多少）。名字里的"门控 (gate)"就是指"零这扇门开多大"。

> **文库大小 (library size) 为什么要乘进去**：不同细胞测序深浅不同（有的总共测到 2000 条、有的 8000 条）。解码器先出各基因的相对占比（加起来＝1），再乘该细胞的总计数，才还原成它真实的计数尺度。
>
> **什么是归一化 (normalization)**：把数值缩放到统一尺度、方便比较。本项目编码器**输入**用的归一化是——先把每个细胞的计数按它的总数缩放（`normalize_total`，消除"测序深浅"差异），再取 `log(1+x)`（`log1p`，把动态范围压小、稳定方差）。可在 §3 的"试一试 2"里亲手看它前后的变化。
>
> **常见坑**：编码器的**输入**是上面这种归一化后的值（`log_variational=True`），但解码器**重建的目标是原始整数计数**——两者别混。且走 ZINB 时 `adata.X` 必须是整数、每细胞总计数 > 0，否则出 NaN。
...
- **分类损失（半监督，可选）——这里藏着 VAE 之外的第二个网络**：如果细胞带类型标签，就在潜向量 z 上再接一个小的**分类器 (classifier)**（几层全连接网络）。它读入 z、输出这个细胞属于各类型的**概率**（如 Tex 80%、Tem 15%…），用**交叉熵 (cross-entropy)** 衡量预测准不准——交叉熵其实就是"真实标签的 NLL"：真实是 Tex，模型给 Tex 的概率越高，损失越小。关于这个分类器，有三点要记住：
  - **它和 VAE 一起训练**：分类损失也加进总损失，梯度既更新分类器、也**回流进编码器**——于是编码器会把潜空间**朝"同类型细胞聚在一起、好分类"的方向**整理。这既提升嵌入质量，也让模型**顺便学会给细胞打标签**。
  - **有什么用**：训练好后来一个新细胞，分类器能直接**预测它的细胞类型**（自动注释）。scAtlasVAE 更进一步用**多个独立分类器**做跨图谱标注对齐（见 1.5）。
  - **一个训练细节**：代码里先把精力用于学好表示，**最后若干轮 (`pred_last_n_epoch`) 才重点训练分类头**——先有好表示、再学分类更稳。
