...
> **深入（可选）· 一个"读代码才知道"、且要读**全**的细节**：论文说预热贯穿训练。真去读 `fit()` 源码（`_gex_model.py:1243` 起）会看到默认 `n_epochs_kl_warmup=400`——**但紧接着（约 1301 行）有一行 `n_epochs_kl_warmup = min(max_epoch, n_epochs_kl_warmup)`**。所以当训练轮数 `max_epoch < 400`（4 万细胞时 =200、11 万细胞时 ≈73），预热长度被**截断成 max_epoch**，`kl_weight = min(1, epoch/max_epoch)`——**λ_KL 恰好在整个训练里从 0 线性爬到 ~1、末轮≈1**。（曾有一版报告只读到函数签名的 400、漏了这行 `min`，误以为"λ_KL 只到 ≈0.18、从没到 1"——我们在 4060 上实跑的 λ_KL 曲线证实它爬到了 ~1，见[阶段 2 图 2-2](phase2_integration_and_benchmark.md)。）含义仍在：**早期 λ_KL 很小、重构（ZINB）主导，末期才到满强度**，这个"先重构后正则"的爬坡正是防坍缩的做法。**教训**：读源码别停在函数签名，要读到它随后怎么改这些参数。这类"论文没写、代码才有"的发现，正是复现的价值。
...
> **动手验证（别只信这句话）**：这个"编码器不看 batch"的论断，代码里有一处**极有说服力的铁证**——去打开 `scAtlasVAE/scatlasvae/model/_gex_model.py`，跳到 **第 966–967 行**，你会看到把 batch 拼进输入的那行 `X = torch.hstack([X, batch_index])` **被注释掉了**。作者写了它、又划掉了——这不是遗漏，是明确的设计。[阶段 3](phase3_reimplement_vae.md) 会带你亲眼看这一行。
