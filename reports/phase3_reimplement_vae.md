...
>
> **关于本篇的源码行号**：下面代码摘录保留的是当时审阅的 scAtlasVAE **上游 pristine 快照**行号，便于对应原始走读；当前本地工作副本已经累积多项兼容性与正确性修复，行号不再是简单“整体 +3”。2026-07 本轮审计时，`encode()` 与被注释的 batch 拼接语句约在 **991–994** 行，`fit()` 约从 **1295** 行开始。以后复查请搜索函数名或具体语句，不要把固定行号当稳定接口。
...
1. **上游摘录第 966–967 行被注释掉了**：`encode` 虽保留 `batch_index` 形参，实际不使用它，所以 scAtlasVAE encoder 不显式接收 batch 元数据。当前工作副本的位置见页首说明。论文把 scVI 概括为 `F(X,B,S)`，但本项目 scvi-tools 默认 `encode_covariates=False` 时同样不接收 batch；scAtlasVAE 的区别是这一接口在结构上固定。无论哪种模型，X 本身仍可能携带批次信号。
...
2. **KL 预热的"真相"**（北极星问题 4 的进阶答案，这一版**更正了旧报告的一处硬错**）：`fit` 签名里确实写着 `n_epochs_kl_warmup=400`，但随后执行 `n_epochs_kl_warmup = min(max_epoch, n_epochs_kl_warmup)`（当前工作副本审计时约在 1353 行）。所以当 `max_epoch < 400`（本项目 10.5 万细胞 `max_epoch=76`；细胞越多 epoch 越少），预热长度被**截断成 max_epoch**；官方实现首轮权重为 0、每轮末递增，最终升到约 1。旧报告说"只到 ≈0.18、从没到 1"是**漏读了这行 `min` 截断**（把预热当成固定 400）。含义仍然成立、只是量级要摆对：**早期 λ_KL 很小、重构（ZINB）主导，末期才升到满强度**。实跑的 λ_KL 曲线（见 phase2 图 2-2）实证了它升到约 1。**这条纠错本身就是"读源码要读全、别停在函数签名"的最佳教材。**
3. **`pred_last_n_epoch=10`**：源码默认只有最后 10 个 epoch 把分类损失加入优化目标。这是**源码默认日程**，不是论文 Methods 明确披露的协议；它可能让表示先由生成目标塑形，但本项目的日程敏感性结果表明不能预先断言“末 10 轮一定更稳”。
...
![官方监督版 vs 手写 UMAP 对照（真实）](figures/fig_phase3_umap_compare.png)

*图 3-4 — **真实对照**：左侧是官方监督版主模型 `X_scAtlasVAE_sup`（历史别名 `X_scAtlasVAE`），右侧是手写最小 VAE，均按 17 个 CD8 亚型上色。**UMAP 朝向是任意的**；图中把手写面板 UMAP1 水平镜像，仅为方便观察。两套 10D latent 的 17 类质心两两距离相关较高（Pearson **0.903**、Spearman **0.895**），说明宏观类间几何相近；投影到各自 2D UMAP 后相关降为 0.633/0.570，说明二维投影会明显扭曲分离观感。主要谱系呈区域富集，但细粒度亚型连续重叠。官方监督版与手写嵌入 kNN 邻域平均 Jaccard=**0.204**。完整结构指标、计算口径与当前四个嵌入的 SHA256 见 [`phase3_structure_metrics.json`](../data/phase3_structure_metrics.json)；长表见 [CSV](../data/phase3_structure_metrics.csv)。*

**记录区（本机实测，~10.5 万全量）**：
```
手写版训练 epoch=76（自动=min(round(20000/N·400),400)）  最终loss≈1198（每细胞，.mean 归一）  有无NaN=无  λ_KL 末值=1.00
官方 vs 手写 kNN Jaccard=0.204
UMAP 定性：Tn/Tem/Temra/Tk/Trm/Tex 等主要谱系占据偏好区域，但细粒度转录状态显著连续重叠；手写版稀有亚型保持稍弱。patient 批次表现需结合 Phase5 的定量 batch score，不能由单张 UMAP 保证
```

> **怎么读 Jaccard=0.204（诚实）**：它远高于独立 30-NN 随机集合的近似期望（约 1.43e-4），说明两套嵌入共享非随机局部结构；但 0.204 也表示它们的精细近邻并不相同。它衡量“两套嵌入是否找到相同邻居”，**不衡量标签是否分开**。现有 raw label ASW 为官方 0.0108、手写 0.0016，证实细亚型本来就大量重叠。按现有 Scanpy `n_neighbors=15` 邻接图计算（该图每细胞实际存 14 个非自身邻居），同标签率为 **0.533/0.526**；若字面重算 15 个非自身近邻则为 **0.532/0.525**，随机同标签基线为 **0.106**。两种口径结论一致：潜空间并未随机坍缩。差异与手写版省去类频率加权 CE、门控稀疏损失和 FCLayer 式 batch 嵌入等有关（见 §11）。全部数字由只读脚本 [`phase3_structure_metrics.py`](../scripts/phase3_structure_metrics.py) 从现有 H5AD/benchmark 重算，不触发训练，并已接入最终门禁。
...
| 验证集/早停/lr 调度 | 每个 epoch 重新切分动态 80/20 train/validation；无有效 early stopping；lr scheduler 可选且默认关闭 | 不切验证集，固定 epoch | 教学最小化；不能把官方实现描述成默认早停 |
| **分类损失日程** | 源码默认只在最后 `pred_last_n_epoch=10` 轮加入优化目标 | 有有效标签时从第 1 轮起每轮加入 | 两者都实现半监督头，但训练轨迹不同；末 10 轮不是论文明确协议 |
| **类别不平衡处理** | 用 `label_category_weight` 做按类别频率加权的交叉熵 | 普通未加权 `CrossEntropyLoss` | 稀有亚型在手写版中的梯度相对更弱，可能影响稀有亚型保持 |
| **编码器/解码器 dropout** | `FCLayer` 默认 `dropout_rate=0.1` | MLP 未放 dropout | 手写版进一步简化了正则化，不能期待逐点复现官方 latent |
| **KL 首轮权重** | 首轮先用 0，每轮结束再递增，末期约到 1 | 首轮用 `1/max_epoch`，末轮为 1 | 总体都是全程线性斜坡，但逐轮权重并不完全相同 |
...
1. **编码器 batch-invariant = `encode()` 中被注释的 batch 拼接语句**——当前工作副本审计时约在 `991–994` 行；以函数/语句搜索为准，这是最硬的一处“设计意图”证据。
...
7. **dropout 门控稀疏损失**：`fit()` 中的 `avg_gate_loss = gate_weight * sigmoid(px_rna_dropout).sum(1).mean()` 被**直接加进总损失**（当前工作副本审计时约在 1465 行）——一个把零膨胀门控整体往下压的稀疏正则（鼓励"少用零膨胀、多让 NB 解释计数"）。论文正文只讲 ZINB 三头，不提这条附加正则；手写最小版没实现它（见 §11 差异清单）。
...
- [x] 已用 `grep` 定位并走读 `encode/decode/forward/fit`，记录每个函数的职责
- [x] 已定位 `encode()` 中被注释的 batch 拼接语句并说明“编码器为何不显式接收 batch”
- [x] 已走读 `decode()` 的 ZINB 三头及文库大小乘入位置
- [x] 已说明 `log_zinb` 中 x=0 / x>0 两种情形及 softplus/lgamma 的数值稳定作用
- [x] 手写版已在 GSE156728 重建的 Zheng CD8 对象上训练，无 `NaN`，得到 `X_minimal`
- [x] 已完成与官方监督版的 UMAP 对照，并报告 Jaccard=0.204 及其有限证据边界；不以 UMAP 外观本身判成功
- [x] 已完成“差异清单”与“代码>论文”两节
