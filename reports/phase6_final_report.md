...
**① 注释迁移（Task 3，论文招牌能力）**：参考集监督训练后，query 数据 zero-shot 自动打标签。两种切法的 zero-shot ROC-AUC——随机留 5%=**0.939**、留整个癌种(UCEC)=**0.903**，双双落在论文 Ext. Data Fig. 2g,h 的区间、且略高于论文 TCellLandscape 的 0.91/0.86（更大参考集→更好迁移），两种设计都**高于 kNN 基线**（0.905 / 0.813），域外泛化领先尤为明显。zero-shot ≈ full-shot（0.939 vs 0.941），印证"不必共训"。
...
**⑤ 跨图谱整合 + 标签对齐（Task 2，scAtlasVAE 独有能力）**：把**全量 Zheng（104,805）**与独立的 **Yost 2019 BCC（12,364）** 联合整合（`batch_hidden_dim=64`、100 epoch、`lr=3e-5`）。用**对不平衡稳健的指标**看：scAtlasVAE 把两图谱整合得明显好于 PCA（**Yost 细胞近邻里 Zheng 占比 24.8% vs PCA 4.0%**；平衡 silhouette 0.041 < 0.087），且**多分类头把两套标签对齐得比 PCA 锐利**（CD8_ex→Tex **0.95 vs 0.64**、CD8_eff→Temra **0.87 vs 0.68**、CD8_mem→Tem 0.62）。这是 scVI/PCA 做不到的并行标签对齐。**三处踩坑（均复查实跑）**：①抄近道（15ep+bhd=10）→假失败；②全量默认 lr 末期梯度爆炸致 NaN 崩→降 `lr=3e-5` 稳住；③Yost 仅占 10.5%，**原始 silhouette 被多数图谱主导会反号**（误显 scAtlasVAE 更差）→改用平衡子采样 + 近邻占比才得正确结论。诚实：整合仍不完美（24.8% ≪ 理想 89%）。详见 [阶段 5 · Task 2](phase5_deeper_validation.md)。
...
**一个方法论亮点**：注释迁移一开始 zero-shot 只有 0.26 准确率、被 kNN 碾压，是**读源码（打补丁后 `_gex_model.py:1372`、`1411`）才发现分类头默认只在最后 `pred_last_n_epoch`(=10) 个 epoch 才训练**（为论文 115 万 atlas 调的），小参考集下远不够；改成全程训练后 AUROC 跃到 0.90+。"读懂 fit 才跑对"正是复现思考性的直接体现。
